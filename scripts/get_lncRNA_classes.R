...
class_summary %>% ggplot(aes(x=Class, y=Numbers, fill = Numbers)) + 
  ...
  coord_flip() +
  scale_fill_gradient(low = '#a5d6fb', high = '#2986cc', name = "Count") +
  labs(x = "LncRNA classes", y = "Count") +
  theme(strip.text = element_text(size = 10), axis.title = element_text(size = 12), 
        axis.text = element_text(size = 10), legend.key.size = unit( 0.75, 'cm'),
